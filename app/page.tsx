...
import { 
  ...
  Building2, Lightbulb, Play, Sparkles, Fingerprint, RefreshCcw, Trash2, Edit3, Wand2, Globe
} from 'lucide-react';
...
export default function Dashboard() {
  ...
  return (
    <div className="flex h-screen bg-[#F8FAFC] text-slate-900 overflow-hidden font-sans relative">
        ...
        <div className="flex items-center gap-3 p-4 bg-slate-800/50 rounded-2xl mb-4 border border-slate-700">
          <div className="flex-1">
            <div className="flex items-center gap-2 mb-1">
               <RefreshCcw size={10} className="text-blue-400 animate-spin" style={{ animationDuration: '4s' }} />
               <p className="text-[10px] font-black text-white uppercase tracking-widest">Network Sync</p>
            </div>
      ...
      {/* MOBILE OVERLAY */}
      {isSidebarOpen && (
        <div className="fixed inset-0 bg-black/50 z-[55] lg:hidden" onClick={() => setIsSidebarOpen(false)} />
      )}
      ...
      <main className="flex-1 flex flex-col lg:flex-row overflow-hidden w-full">
            ...
            <div className="animate-in fade-in slide-in-from-left-4 duration-500">
              <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-10 gap-4 mt-12 lg:mt-0">
                ...
                <div className="flex gap-3 w-full md:w-auto">
                    <button onClick={handleGenerateFullDemo} disabled={isLoading} className="flex-1 md:flex-none flex items-center justify-center gap-2 px-6 py-4 bg-indigo-50 text-indigo-600 rounded-2xl font-black uppercase text-[10px] border border-indigo-100 hover:bg-indigo-100 transition disabled:opacity-50">
              ...
              {error && (
                <div className="bg-red-50 border border-red-100 p-4 rounded-2xl flex items-center gap-3 text-red-700 text-xs font-bold mb-6">
              ...
              <div className="space-y-4">
                ...
                {interactions.length > 0 ? (
                  ...
                  <div className="text-center py-24 bg-slate-50/50 border-2 border-dashed border-slate-200 rounded-[40px] opacity-40">
                    <p className="text-slate-400 text-sm font-bold uppercase tracking-widest italic">Vault Empty. Generate an AI Story or inject data.</p>
                  </div>
                )}
              </div>
            </div>
        ...
        {/* AI PANEL (Stacks on mobile, stays on right on desktop) */}
        <aside className="w-full lg:w-[480px] bg-white p-6 lg:p-10 overflow-y-auto shadow-2xl border-t lg:border-t-0 lg:border-l border-slate-200">
          {selectedDeal && aiStrategy && !isAnalyzing && (
            <div className="space-y-8 animate-in fade-in duration-700">
              <div className="flex items-center justify-between">
                ...
                <button 
                  ...
                  className={`flex items-center gap-2 px-3 py-1.5 rounded-full text-[9px] font-black uppercase transition-all ${showExplanation ? 'bg-blue-600 text-white shadow-lg' : 'bg-slate-100 text-slate-400 hover:bg-slate-200'}`}
                >
              ...
              <div className="bg-slate-900 p-10 rounded-[48px] text-white shadow-2xl relative overflow-hidden">
                <div className="flex justify-between items-start">
                  ...
                  <div className="flex items-center gap-1.5 px-2 py-1 bg-blue-500/20 rounded-full border border-blue-500/30">
                    ...
                    <span className="text-[8px] font-bold text-blue-300 uppercase">Vector Matching Engine</span>
                  </div>
                </div>
                ...
                <div className="mt-6 flex gap-4 border-t border-slate-800 pt-4 opacity-70">
                    ...
                    <div className="text-[9px] font-black uppercase tracking-widest"><span className="text-blue-400">{aiStrategy.similar_deal_count}</span> Semantic Matches</div>
                </div>
                ...
                <div className="space-y-4 animate-in slide-in-from-right-4 duration-500">
                  <div className="bg-slate-900 border border-blue-500/30 rounded-[32px] p-6 shadow-2xl">
                    <div className="flex items-center gap-2 mb-4">
                      ...
                      <h4 className="text-[10px] font-black text-white uppercase tracking-widest">Semantic Audit Trail</h4>
                    </div>
              ...
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <button onClick={() => handleMarkOutcome('won')} className="py-4 bg-green-50 text-green-700 border border-green-200 rounded-2xl text-[10px] font-black uppercase hover:bg-green-100 transition">Mark Won</button>
                  <button onClick={() => handleMarkOutcome('lost')} className="py-4 bg-red-50 text-red-700 border border-red-200 rounded-2xl text-[10px] font-black uppercase hover:bg-red-100 transition">Mark Lost</button>
                </div>
                <button onClick={() => setMeetingPrep(aiStrategy.script)} className="w-full py-6 bg-blue-600 text-white rounded-[32px] font-black uppercase tracking-[0.2em] text-[11px] shadow-xl hover:bg-blue-700 transition-all active:scale-95 shadow-blue-100">Prepare Battle Script</button>
                {meetingPrep && (
        ...
        <div className="fixed inset-0 bg-slate-900/90 backdrop-blur-md flex items-center justify-center p-6 z-[100] text-slate-900">
          <div className="bg-white rounded-[48px] w-full max-w-2xl shadow-2xl overflow-hidden animate-in zoom-in-95">
            <div className="p-8 lg:p-12 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
              <h3 className="text-2xl lg:text-3xl font-black tracking-tighter uppercase italic">Commit Interaction</h3>
            ...
            <div className="p-8 lg:p-12 space-y-6">
              ...
              <button onClick={handleSaveInteraction} disabled={isLoading || !transcript} className="w-full py-6 lg:py-8 bg-blue-600 text-white rounded-[40px] font-black uppercase tracking-[0.2em] text-sm disabled:opacity-50 shadow-2xl flex items-center justify-center gap-4">
